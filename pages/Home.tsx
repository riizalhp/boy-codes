...
const FaqItem = ({ question, answer }: { question: string, answer: string }) => {
  ...
  return (
    <div className="border-b border-brand-white/10 py-6">
      ...
      >
        <h4 className="text-lg font-semibold font-heading text-brand-white hover:text-brand-green transition-colors">{question}</h4>
        <ChevronDown
          size={24}
      ...
      >
        <div className="overflow-hidden">
          <p className="text-brand-white/80">{answer}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="bg-brand-dark">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 z-10">
          <AnimatedDiv>
            <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold font-heading tracking-tight leading-tight">
              Jasa Pembuatan Website Profesional <br className="hidden md:block" /> di <span className="text-brand-green text-glow">Yogyakarta</span>
            </h1>
            <p className="mt-6 max-w-3xl mx-auto text-lg sm:text-xl text-brand-white/80">
              Bikin website yang menghasilkan untuk UMKM, personal brand, dan perusahaan. Desain modern, SEO-friendly, dan siap membantu bisnismu berkembang.
            </p>
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
              <div>
                ...
                <p className="text-xs text-brand-white/60 mt-2">Gratis 30 menit ngobrol strategi bareng Mas Boy</p>
              </div>
      ...
      <Section id="tentang" className="bg-brand-gray">
        <AnimatedDiv className="grid md:grid-cols-2 gap-12 items-center">
            <div className="text-center md:text-left">
                <h2 className="text-3xl lg:text-4xl font-bold font-heading">Mas Boy itu <span className="text-brand-green">Partner Digital</span> Kamu.</h2>
                <p className="mt-4 text-lg text-brand-white/70">Bukan cuma coder, tapi partner strategis yang ngerti bisnis kamu. Dari personal branding sampai pembuatan toko online, Mas Boy siap bikin kamu percaya diri di dunia online.</p>
            </div>
      ...
      <Section id="solusi">
        <AnimatedDiv className="text-center">
          <h2 className="text-3xl lg:text-4xl font-bold font-heading">Sering Ngerasain Ini?</h2>
          <p className="mt-4 max-w-2xl mx-auto text-lg text-brand-white/70">Kamu nggak sendirian. Ini beberapa masalah yang sering klien Mas Boy alamin sebelum kami bantu.</p>
        </AnimatedDiv>
          ...
          {MASALAH_ITEMS.map((item, index) => (
            <AnimatedDiv key={index}>
              <div className="bg-brand-gray p-8 rounded-xl border border-brand-white/10 h-full transform hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand-green/10 transition-all duration-300 text-center">
                ...
                <h3 className="text-xl font-bold font-heading text-brand-green">{item.title}</h3>
                <p className="mt-3 text-brand-white/70">{item.description}</p>
              </div>
            </AnimatedDiv>
          ))}
        </div>
      ...
      <Section id="layanan" className="bg-brand-gray">
          <AnimatedDiv className="text-center">
            <h2 className="text-3xl lg:text-4xl font-bold font-heading">Solusi Lengkap Sesuai Kebutuhan</h2>
            <p className="mt-4 max-w-2xl mx-auto text-lg text-brand-white/70">Baik kamu perorangan, UMKM, atau perusahaan besar, Mas Boy punya solusi jasa bikin website yang pas.</p>
          </AnimatedDiv>
            ...
            {LAYANAN_SEGMENTS.map((item, index) => (
              <AnimatedDiv key={index}>
                 <div className="bg-brand-dark p-8 rounded-xl h-full flex flex-col">
                    ...
                    <p className="mt-2 text-brand-white/70 flex-grow">{item.description}</p>
      ...
      <Section id="kenapa-kami">
        <AnimatedDiv className="max-w-5xl mx-auto text-center">
            <h2 className="text-3xl lg:text-4xl font-bold font-heading">Kenapa Harus <span className="text-brand-green">Mas Boy</span>?</h2>
            <p className="mt-4 text-lg text-brand-white/80">Karena website kamu harus kerja 24/7 buat bisnis, bukan cuma jadi pajangan.</p>
                ...
                {WHY_US_POINTS.map((item, index) => (
                    <div key={index} className="group bg-brand-gray p-6 rounded-xl border border-brand-white/10 hover:border-brand-green/30 hover:shadow-lg hover:shadow-brand-green/10 transition-all duration-300 transform hover:-translate-y-1 text-left h-full">
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                <h4 className="font-bold font-heading text-lg">{item.title}</h4>
                                <p className="text-sm text-brand-white/70 mt-1">{item.description}</p>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className="mt-16 pt-12 border-t border-brand-white/10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
                    <AnimatedStat icon={<Star size={24} />} end={50} suffix="+" label="Website Dibangun" />
                    <AnimatedStat icon={<ThumbsUp size={24} />} end={100} suffix="%" label="Client Puas" />
                    <AnimatedStat icon={<Clock size={24} />} end={98} suffix="%" label="Tepat Waktu" />
                </div>
            </div>
          </AnimatedDiv>
      </Section>
      ...
      <Section id="proses">
        <AnimatedDiv className="text-center">
          <h2 className="text-3xl lg:text-4xl font-bold font-heading">Proses Kerja Mas Boy</h2>
          <p className="mt-4 max-w-2xl mx-auto text-lg text-brand-white/70">Transparan, kolaboratif, dan fokus pada hasil. Begini cara kita bekerja sama.</p>
        </AnimatedDiv>
                ...
                <div className="flex items-center" style={{ flexDirection: index % 2 === 0 ? 'row' : 'row-reverse' }}>
                  <div className="w-1/2 px-6">
                    <div className={`text-center md:text-${index % 2 === 0 ? 'right' : 'left'}`}>
                      ...
                      <p className="mt-2 text-brand-white/80">{step.description}</p>
                      <p className="mt-3 text-sm text-brand-green/80 italic">"{step.note}"</p>
                    </div>
                  </div>
      ...
      <Section id="showcase" className="bg-brand-gray">
        <AnimatedDiv className="text-center">
          <h2 className="text-3xl lg:text-4xl font-bold font-heading">Portofolio & <span className="text-brand-green">Hasil Karya Terbaik</span></h2>
          <p className="mt-4 max-w-2xl mx-auto text-lg text-brand-white/70">Beberapa proyek pilihan yang sudah bikin klien bangga dan bisnisnya berkembang.</p>
        </AnimatedDiv>
      ...
      <Section className="py-20 sm:py-24 bg-brand-dark">
        <AnimatedDiv className="text-center">
            <h3 className="text-sm font-bold uppercase tracking-widest text-brand-white/60">DIPERCAYA OLEH BRAND TERKEMUKA</h3>
        </AnimatedDiv>
      ...
      <Section id="testimoni" className="bg-brand-gray">
        <AnimatedDiv className="text-center">
          <h2 className="text-3xl lg:text-4xl font-bold font-heading">Kata Mereka Tentang Mas Boy</h2>
          <p className="mt-4 max-w-2xl mx-auto text-lg text-brand-white/70">Testimoni asli dari klien yang sudah merasakan dampak positif dari jasa pembuatan website kami.</p>
        </AnimatedDiv>
      ...
      <Section id="faq">
        <AnimatedDiv className="text-center">
          <h2 className="text-3xl lg:text-4xl font-bold font-heading">Pertanyaan Umum (FAQ)</h2>
          <p className="mt-4 max-w-2xl mx-auto text-lg text-brand-white/70">Punya pertanyaan? Mungkin jawabannya ada di sini. Jika tidak, jangan ragu untuk menghubungi Mas Boy.</p>
        </AnimatedDiv>
      ...
      <Section>
        <AnimatedDiv className="text-center bg-brand-gray rounded-xl p-12 lg:p-20">
          <h2 className="text-3xl lg:text-5xl font-bold font-heading">Siap Punya <span className="text-brand-green">Website Profesional</span> yang Benar-Benar Bekerja?</h2>
          <p className="mt-4 max-w-xl mx-auto text-lg text-brand-white/70">Jangan tunda lagi. Ngobrol gratis nggak dipungut biaya, malah bisa dapet pencerahan.</p>
          <div className="mt-8">
            <Link to="/kontak" className="inline-block bg-brand-green text-brand-dark font-bold font-heading py-4 px-10 rounded-lg text-xl hover:bg-opacity-80 transition-all duration-300 button-glow">
              Ayo Ngobrol Gratis Sekarang
            </Link>
            <p className="text-xs text-brand-white/60 mt-3">Slot konsultasi gratis terbatas setiap bulan!</p>
          </div>
        </AnimatedDiv>
      </Section>
    </div>
